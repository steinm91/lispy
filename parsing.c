...
// declare new struct for lisp value (lval)
typedef struct lval {
    int type;
    long num;
    char* err;
...
lval* lval_err(char* message) {
    lval* v = malloc(sizeof(lval));
    v->type = LVAL_ERR;
    v->err = malloc(strlen(message) + 1); // strlen excludes null terminator, so we add 1 to allocate enough space
...
// create pointer to a new empty s-expression lisp value
...
void lval_del(lval* v) {
    switch(v->type) {
        // do nothig special for number types
        case LVAL_NUM: 
            break;

        // free the string data for errors and symbols
        case LVAL_ERR:
            free(v->err);
            break;
        case LVAL_SYM:
        ...
        // delete all elements inside an s-expression
        case LVAL_SEXPR:
            for (int i = 0; i < v->count; i++) {
                lval_del(v->cell[i]);
            }
            // don't forget to free the memory allocated to contain the pointers to the elements
            free(v->cell);
            break;
    }
    ...
    while (a->count > 0) {
        ...
        if (strcmp(op, "/") == 0) {
            if (y->num == 0) {
                lval_del(x);
                lval_del(y);
                x = lval_err("Division by zero is not allowed!");
                break;
            }
            x->num /= y->num;
        }
